...
if __name__ == '__main__':
    ...
    if '3' in args.steps or '4' in args.steps:
        ...
        for fn in os.listdir(frames_dir):
            ...
            top_left_x, top_left_set_x, top_left_y, top_left_second_y, delta_x, delta_y = video_metadata[-8], video_metadata[-7], video_metadata[-6], video_metadata[-5], video_metadata[-4], video_metadata[-3]
            is_score_black_on_white = False
            if video_metadata[-2] == 'w':
                is_score_black_on_white = True
            ...
            training_set_creation_knn_new.main(input_frame_dir, score_subdir, result_subdir, int(top_left_x), int(top_left_set_x), int(top_left_y), int(top_left_second_y), int(delta_x), int(delta_y), is_score_black_on_white, is_top_player_top, debug=True)
