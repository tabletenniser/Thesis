...
# START_FRAME=389
TRAINING_SET_PATH = 'training_set_large'
START_FRAME=104
END_FRAME=3000
    ...
    while (os.path.exists(input_frame_file) and index<=END_FRAME):
        ...
        try:
            ...
            if valid(num_1) and valid(num_2) and set_valid(num_set1) and set_valid(num_set2):
                ...
                # Save images to proper training set location
                im = Image.open(input_frame_file)
                im1 = im.crop((TOP_LEFT_X, TOP_LEFT_Y, TOP_LEFT_X+DELTA_X, TOP_LEFT_Y+DELTA_Y)).convert('L')
                im1.save(os.path.join(TRAINING_SET_PATH, str(num_int_1), 'frame_%05d_1.png'%index))
                print 'saving to training_set: ', os.path.join(TRAINING_SET_PATH, str(num_int_1), 'frame_%05d_1.png'%index)
                im2 = im.crop((TOP_LEFT_X, TOP_LEFT_Y+DELTA_Y, TOP_LEFT_X+DELTA_X, TOP_LEFT_Y+2*DELTA_Y)).convert('L')
                im2.save(os.path.join(TRAINING_SET_PATH, str(num_int_2), 'frame_%05d_2.png'%index))
                print 'saving to training_set: ', os.path.join(TRAINING_SET_PATH, str(num_int_2), 'frame_%05d_2.png'%index)
                im_set1 = im.crop((TOP_LEFT_X+DELTA_X, TOP_LEFT_Y, TOP_LEFT_X+2*DELTA_X, TOP_LEFT_Y+DELTA_Y)).convert('L')
                im_set1.save(os.path.join(TRAINING_SET_PATH, str(num_int_set1), 'frame_%05d_set1.png'%index))
                print 'lalal:saving to training_set: ', os.path.join(TRAINING_SET_PATH, str(num_int_set1), 'frame_%05d_set1.png'%index)
                im_set2 = im.crop((TOP_LEFT_X+DELTA_X, TOP_LEFT_Y+DELTA_Y, TOP_LEFT_X+2*DELTA_X, TOP_LEFT_Y+2*DELTA_Y)).convert('L')
                im_set2.save(os.path.join(TRAINING_SET_PATH, str(num_int_set2), 'frame_%05d_set2.png'%index))
                # END: Save images to proper training set location
